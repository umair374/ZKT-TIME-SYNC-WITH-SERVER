...
namespace TimeSyncService.Utilities
{
    public class ZkemClient : IZKEM
    {
        ...
        public void SetDeviceTime(int deviceId, DateTime serverTime)
        {
            if (objCZKEM != null)
            {
                try
                {
                    bool result = objCZKEM.SetDeviceTime2(
                        deviceId,
                        serverTime.Year,
                        serverTime.Month,
                        serverTime.Day,
                    ...
                    if (result)
                    {
                        Console.WriteLine($"Device time updated successfully to: {serverTime}");
                    }
                    else
                    {
                        Console.WriteLine("Failed to update device time.");
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error updating device time: {ex.Message}");
                }
            }
        }
